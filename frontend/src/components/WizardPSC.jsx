...
export default function WizardPSC() {
  ...
  const loadFromAnagrafica = (id, tipo) => {
    if (tipo === 'committente') {
      ...
      if (!c) return;
      setForm(f => ({ ...f, committente_tipo: c.tipo, committente_nome: c.nome, committente_cognome: c.cognome, committente_ragione_sociale: c.ragione_sociale, committente_cf: c.codice_fiscale, committente_piva: c.piva, committente_indirizzo: c.indirizzo, committente_citta: c.citta, committente_telefono: c.telefono, committente_email: c.email }));
    }
    if (tipo === 'csp' || tipo === 'cse') {
      const c = coordinatori.find(x => x.id === parseInt(id));
      if (!c) return;
      const p = tipo === 'csp' ? 'csp_' : 'cse_';
      setForm(f => ({ ...f, [`${p}nome`]: c.nome, [`${p}cognome`]: c.cognome, [`${p}ordine`]: c.ordine_professionale, [`${p}numero_ordine`]: c.numero_ordine, [`${p}data_aggiornamento`]: c.data_aggiornamento, [`${p}pec`]: c.pec }));
    }
  };
  ...
  return (
    <div>
      <div className="page-header"><h1>📗 Piano di Sicurezza e Coordinamento</h1><p>Art. 100 — D.Lgs. 81/2008 — Allegato XV</p></div>
        ...
        <div className="wizard-body">
          ...
          {step === 0 && (
            <>
              ...
              <div className="form-grid">
                <F label="N. imprese esecutrici previste" field="num_imprese" type="number" hint="Includere subappaltatori e lavoratori autonomi con mezzi propri" placeholder="es. 3" />
          ...
          {step === 1 && (
            <>
              ...
              {checkResult && (
                <div className={checkResult.obbligatorio ? 'info-box' : 'warn-box'}>
              ...
              <div className="form-group">
                ...
                <select className="form-control" value={form.natura_opera || ''} onChange={e => set('natura_opera', e.target.value)}>
                  ...
                  {['Nuova costruzione residenziale','Nuova costruzione commerciale/industriale','Ristrutturazione edilizia','Manutenzione straordinaria','Demolizione','Opere stradali'].map(o => <option key={o}>{o}</option>)}
                </select>
              </div>
            ...
            <>
              ...
              {committenti.length > 0 && (
                <div className="form-group">
                  ...
                  <select className="form-control" defaultValue="" onChange={e => loadFromAnagrafica(e.target.value, 'committente')}>
                    <option value="">— Seleziona —</option>
                    {committenti.map(c => <option key={c.id} value={c.id}>{c.tipo === 'persona_giuridica' ? c.ragione_sociale : `${c.nome || ''} ${c.cognome || ''}`}</option>)}
                  </select>
                </div>
              )}
              <div className="form-grid">
              ...
              {['csp','cse'].map(ruolo => (
                <div key={ruolo}>
                  <div className="section-divider">{ruolo === 'csp' ? 'CSP — Coordinatore Progettazione' : 'CSE — Coordinatore Esecuzione'}</div>
                  {coordinatori.length > 0 && (
                    <div className="form-group">
                      <label className="form-label">📁 Carica da anagrafica</label>
                      <select className="form-control" defaultValue="" onChange={e => loadFromAnagrafica(e.target.value, ruolo)}>
                        <option value="">— Seleziona —</option>
                  ...
                  <div className="form-grid">
                    ...
                    <F label="Data aggiornamento" field={`${ruolo}_data_aggiornamento`} type="date" hint="Aggiornamento 40h ogni 5 anni" />
            ...
            <>
              ...
              {imprese.length > 0 && (
                <div className="form-group">
                  ...
                  <select className="form-control" defaultValue="" onChange={e => { addImpresa(e.target.value); e.target.value = ''; }}>
                  ...
                  {impreseSel.map(imp => (
                    <div key={imp.id} style={{ border: '1.5px solid var(--border)', borderRadius: 8, padding: 16 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                        ...
                        <button className="btn btn-danger btn-sm" onClick={() => setImpreseSel(p => p.filter(x => x.id !== imp.id))}>✕ Rimuovi</button>
                      </div>
                      <div className="form-group" style={{ margin: 0 }}>
                        <label className="form-label">Attività nel cantiere</label>
                        <input className="form-control" placeholder="es. Strutture in c.a., murature..."
                          value={imp.attivita || ''}
            ...
            <>
              ...
              <div className="form-grid">
                <div className="form-group">
                  ...
                  <select className="form-control" value={form.rischi_allegato_xi_desc || ''} onChange={e => set('rischi_allegato_xi_desc', e.target.value)}>
            ...
            <>
              ...
              <div className="form-grid">
                ...
                <div className="form-group" style={{ background: '#EEF4FA', padding: 12, borderRadius: 8 }}>
                  ...
                  <input type="number" className="form-control" value={form.costi_sicurezza || ''} onChange={e => set('costi_sicurezza', e.target.value)} style={{ fontWeight: 700 }} />
                </div>
              </div>
              <div className="wizard-nav">
            ...
            <>
              ...
              <div style={{ background: '#F8F5F0', borderRadius: 8, padding: 20, fontSize: '0.875rem', marginBottom: 20 }}>
                ...
                {[
                  ...
                  ['Costi sicurezza', form.costi_sicurezza ? `€ ${form.costi_sicurezza}` : '—'],
                ].map(([k, v]) => v && (
              ...
              <p style={{ fontSize: '0.82rem', color: '#5A6B7D', marginBottom: 20 }}>
                L'AI genererà automaticamente le sezioni narrative (analisi rischi, organizzazione cantiere, misure di coordinamento) conformi all'Allegato XV.
              </p>
              <div className="wizard-nav">
          ...
          {step === 7 && (
            <div style={{ textAlign: 'center', padding: '32px 0' }}>
              ...
              <p style={{ color: '#5A6B7D', marginBottom: 24, maxWidth: 460, margin: '0 auto 24px' }}>
                Il Piano di Sicurezza e Coordinamento è stato generato con contenuto AI-assistito conforme all'Allegato XV. Verificalo e firmalo prima dell'utilizzo.
              </p>
              {docId && (
